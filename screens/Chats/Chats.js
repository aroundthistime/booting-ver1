...
import { GET_CHATS, QUIT_CHAT } from "./ChatQueries";
                    ...
                    <ChatTabsContainer>
                        {data.getChats.map((chat) => {
                            ...
                            if (chat.participants.length > 1){
                                if (chat.participants[0].id === userObj.id){
                                    opponent = chat.participants[1];
                                } else {
                                    opponent = chat.participants[0];
                                }
                            } else {
                                opponent = DEFAULT_USER;
                            }
                            return (
                                <ChatTab key={chat.id} onPress={() => navigation.navigate("Chat", {chatId : chat.id})} onLongPress={() => {confirmQuitChat(chat.id, opponent.name)}}>
            ...
            {/* <ScrollView style={{flex : 1, backgroundColor : styles.bgColor, paddingTop : 5}}>
                <ChatTabsContainer>
                    <ChatTab onPress={() => navigation.navigate("Chat")} onLongPress={() => Alert.alert("HI")}>
                        ...
                        <ChatInfoSub>
                            <ChatTimestamp>2021년 12월 18일</ChatTimestamp>
                            <ChatUnreadMark />
                        </ChatInfoSub>
                    </ChatTab>
                </ChatTabsContainer>
                <ChatTab onPress={() => navigation.navigate("Chat")} onLongPress={() => Alert.alert("HI")}>
                    ...
                    <ChatInfoSub>
                        <ChatTimestamp>17:51</ChatTimestamp>
                        <ChatUnreadMark />
                    </ChatInfoSub>
                </ChatTab>
            </ScrollView> */}
